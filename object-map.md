...
| Script `Game.Meta` | 45 | **51** (tăng từ 46 — thêm `Endgame/ArenaSystem.cs` + `Endgame/ArenaScreen.cs` [task-arena.md] + `Accessibility/TextScaleApplier.cs` [task-accessibility.md] + `Notifications/RedDotService.cs` [task-reddot.md] + `Gacha/GachaInfoScreen.cs` [task-gacha-disclosure.md]) | 🟡 P4 |
...
| Prefab | ~70 | **18** (tăng từ 16 — thêm `UI_Arena` [task-arena.md] + `UI_GachaInfo` [task-gacha-disclosure.md, clone `UI_Codex`]) | 🔴 |
...
| Test file | 28 | **47** (tăng từ 43 — thêm `Meta/ArenaSystemTests.cs` [8 test], `Meta/RedDotServiceTests.cs` [10 test, task-reddot.md], `Combat/BossSignatureMoveTests.cs` [5 test] + `Combat/BossPhaseEnrageTests.cs` [6 test, cả 2 task-boss-phase-enrage.md]) | 🟢 P0 — **668/668 test xanh** ✓ đếm lại thật qua `run_tests` 2026-08-18 (con số tuyệt đối giữa các mốc không cộng dồn khớp 1-1 được nữa vì repo đã gộp lại thành 1 `Initial commit` ở đâu đó giữa chừng — che mất lịch sử test-theo-task trước điểm đó; **dùng số tuyệt đối đo trực tiếp mỗi lần, đừng cộng dồn theo delta cũ**) |
